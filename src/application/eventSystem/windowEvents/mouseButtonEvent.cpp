...
namespace systemEvent {
	...
	std::shared_ptr<MouseButtonHandler> makeMouseButtonHandler(void(*function)(int, int, int)) {
		...
		public:
			Adapter(myFunPtr function) : MouseButtonHandler(MOUSE_BUT_GENERAL), m_FunPtr(function) { }
		public:
	...
	std::shared_ptr<MouseButtonHandler> makeMouseButtonPushHandler(void(*function)(int, int)) {
		...
		public:
			Adapter(myFunPtr function) : MouseButtonHandler(MOUSE_BUT_PUSH), m_FunPtr(function) { }
		public:
			void handle(int button, int action, int mods) const override {
	...
	std::shared_ptr<MouseButtonHandler> makeMouseButtonReleseHandler(void(*function)(int, int)) {
		...
		public:
			using myFunPtr = void(*)(int, int);
		public:
			Adapter(myFunPtr function) : MouseButtonHandler(MOUSE_BUT_RELESE), m_FunPtr(function) { }
		public:
			void handle(int button, int action, int mods) const override {
				(*m_FunPtr)(button, mods);
			}
