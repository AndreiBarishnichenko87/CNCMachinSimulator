...
namespace systemEvent {
	...
	std::shared_ptr<KeyboardHandler> makeKeyboardRepeatHandler(void(*function)(int, int, int), EventHandlingMode handlingMode) {
		
		class Adapter : public KeyboardHandler {
		public:
			using myFunPtr = void(*)(int, int, int);
		public:
			Adapter(myFunPtr function, EventHandlingMode handlingMode) : KeyboardHandler(EventType::KeyRepeat, handlingMode), m_FunPtr(function) { }
		public:
			void handle(int key, int scancode, int mods) const override {
				(*m_FunPtr)(key, scancode, mods);
			}
